#print("""Xin lỗi ae!! Tool trên py chính thức dừng hoạt động lúc 11h30 ngày 4/4
#Vui lòng truy cập 
#toololm.hackertrashman.workers.dev
#để tiếp tục sử dụng
#Trân trọng cảm ơn ae đã ủng hộ mình""")
#UPDATE19
...
_UA_KEY='ua'; _LG_KEY='lang'; _DID_KEY='device_id'; _CV_KEY='cv'
_DEFAULT_UA='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36'
_DEFAULT_LANG='vi-VN,vi;q=0.9,en-US;q=0.8,en;q=0.7'

def _dev_profile():
    d=os.path.join(os.path.expanduser('~'),'.olmdata'); os.makedirs(d,exist_ok=True)
    p=os.path.join(d,'.dp.json')
    _required={_UA_KEY,_LG_KEY,_DID_KEY,_CV_KEY}
    try:
        if os.path.exists(p):
    ...
    prof={_UA_KEY:ua,_LG_KEY:lg,_DID_KEY:did,_CV_KEY:cv}
    try:
...
def _acc_profile(uname):
    b=_dev_profile()
    did=b.get(_DID_KEY) or hashlib.sha256(os.urandom(16)).hexdigest()[:32]
    h=hashlib.md5(f'{did}_{uname}'.encode()).hexdigest()
    return {**b,_DID_KEY:h}

def _ensure_prof():
    global _prof,HEADERS
    if not isinstance(_prof,dict) or not _prof.get(_UA_KEY):
        _prof=_acc_profile('__init__'); HEADERS=_hdrs()

_prof=_acc_profile('__init__')

def _hdrs():
    ua=_prof.get(_UA_KEY,_DEFAULT_UA); lg=_prof.get(_LG_KEY,_DEFAULT_LANG)
    return {'user-agent':ua,'accept':'application/json, text/javascript, */*; q=0.01','accept-language':lg,'x-requested-with':'XMLHttpRequest','origin':'https://olm.vn','referer':'https://olm.vn/'}
