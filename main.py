...
def hide_cursor():
    if not IS_ANDROID: print('\033[?25l',end='',flush=True)
def show_cursor():
    if not IS_ANDROID: print('\033[?25h',end='',flush=True)
def clear_line():
...
def save_accounts(accounts):
    try:
        with open('accounts.json','w',encoding='utf-8') as f: json.dump(accounts,f,ensure_ascii=False,indent=2)
        return True
    except: return False
...
def main_menu(session,user_id,user_name,is_vip,remaining_uses):
    ...
    while True:
        ...
        if choice=='1':
            ...
            time.sleep(0.5 if IS_ANDROID else 1);sp.stop(True,'Đã đăng xuất');pause();return False,remaining_uses
...
def main():
    ...
    print();time.sleep(0.2 if IS_ANDROID else 0.4)
    print_tutorial()
    while True:
